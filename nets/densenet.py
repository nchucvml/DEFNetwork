    ...
    with tf.variable_scope(scope, 'densenetxxx', [inputs, num_classes],
        ...
        with slim.arg_scope([slim.batch_norm, slim.dropout],
            ...
            with tf.variable_scope('final_block', [inputs]):
                net = slim.batch_norm(net)
                net = tf.nn.relu(net)
                net = slim.dropout(net, 0.5, is_training=is_training, scope='dropout_d1')
                net = slim.conv2d(net, 512, [7, 7], padding='VALID', scope='fc_d1')
...
def densenet121(inputs, num_classes=1000, is_training=None, data_format='NHWC',  reuse=None):
